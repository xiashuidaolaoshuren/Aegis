...
- [ ] **Do:** GitHub Actions (or equivalent) Clang build/test; AddressSanitizer-capable preset documented. No WebEngine.
- **Blocked by:** T1
- **Plan mode:** medium
- **TDD suitable:** no
...
- [ ] **Do:** Header-only (or small cpp) types: `Result`, `Tagged`, `Money`/`Currency`, wallet/terminal/STAN/RRN/`Pan` with masked format and explicit full-PAN accessor.
- **Blocked by:** T1
- **Plan mode:** high
- **TDD suitable:** yes
